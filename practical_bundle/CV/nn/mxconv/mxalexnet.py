...
class MxAlexNet:
	...
	def build(classes):
		...
		#不用width等参数是因为ImageRecordIter自动推断图片维度
		...
		bn1 = mx.sym.BatchNorm(data=act1_1)
		pool1 = mx.sym.Pooling(data=bn1_1, pool_type="max",
			kernel=(3,3), stride=(2,2))
		do1 = mx.sysm.Dropout(data=pool1, p=0.25)
		...
		do5 = mx.sym.Dropout(data=bn5_1, p=0.5)#FC层的dropout 40到50
